...
python scripts/ci/package_librsvg_materials.py $bundle
if ($LASTEXITCODE -ne 0) { throw "librsvg Cargo source and license collection failed" }
python scripts/ci/package_quickjs_materials.py $bundle
...
python scripts/ci/package_windows_source_access.py $bundle
if ($LASTEXITCODE -ne 0) { throw "Windows Canvas source separation failed" }
