...
cp -a "${provider_source}/." "${tools}/bgutil-provider/"
...
curl --fail --location --silent --show-error "https://raw.githubusercontent.com/yt-dlp/yt-dlp/${YTDLP_VERSION}/THIRD_PARTY_LICENSES.txt" --output "${licenses}/yt-dlp-THIRD_PARTY_LICENSES.txt"
mkdir -p "${bundle}/sources"
curl --fail --location --silent --show-error "https://github.com/yt-dlp/yt-dlp/releases/download/${YTDLP_VERSION}/yt-dlp.tar.gz" --output "${bundle}/sources/yt-dlp.tar.gz"
printf '%s  %s\n' "072aad4f2a7604e92155f61a275a4752dc64046c8f6d90df3710525d94cd37c1" "${bundle}/sources/yt-dlp.tar.gz" | sha256sum --check --strict -
...
python3 scripts/ci/package_linux_native_materials.py --bundle "${bundle}"
python3 scripts/ci/package_quickjs_materials.py "${bundle}"
python3 scripts/ci/package_ytdlp_notices.py "${bundle}" --platform "${ARTIFACT_SUFFIX}"
