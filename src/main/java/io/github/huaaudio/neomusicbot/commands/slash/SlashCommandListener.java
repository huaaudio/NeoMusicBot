...
import io.github.huaaudio.neomusicbot.audio.PlaybackLoadToken;
import io.github.huaaudio.neomusicbot.audio.QueuedTrack;
import io.github.huaaudio.neomusicbot.audio.RequestMetadata;
import io.github.huaaudio.neomusicbot.audio.media.MediaResolverReadiness;
import io.github.huaaudio.neomusicbot.audio.media.SensitiveLogSanitizer;
import io.github.huaaudio.neomusicbot.audio.media.PersistentMediaReferencePolicy;
...
public final class SlashCommandListener extends ListenerAdapter
{
    ...
    public void onReady(@NotNull ReadyEvent event)
    {
        ...
        String developmentGuild = BotEnvironment.value("NEOMUSICBOT_COMMAND_GUILD_ID");
        if (developmentGuild != null && !developmentGuild.isBlank())
        {
            ...
            {
                LOG.error("NEOMUSICBOT_COMMAND_GUILD_ID is not a valid Discord snowflake");
                return;
            }
            if (guild == null)
            {
                LOG.error("NEOMUSICBOT_COMMAND_GUILD_ID does not identify a guild visible to this bot");
                return;
            }
    ...
    {
        String invite = event.getJDA().getInviteUrl(NeoMusicBot.RECOMMENDED_PERMS);
    ...
    private void owner(SlashCommandInteractionEvent event)
    {
        ...
        switch (event.getSubcommandName())
        {
            ...
            case "shutdown" -> event.reply(warn("Shutting down NeoMusicBot...")).setEphemeral(true).queue(
        ...
        event.deferReply(true).queue(ignored ->
        {
            try
            {
                bot.getThreadpool().execute(() ->
                {
                    ...
                    {
                        edit(event, fail("The default playlist cannot be changed while NeoMusicBot is shutting down."));
                    }
                });
            }
            catch(RejectedExecutionException ex)
    ...
    private void ownerDebug(SlashCommandInteractionEvent event)
    {
        event.deferReply(true).queue(ignored ->
        {
            try
            {
                bot.getThreadpool().execute(() ->
                {
                    ...
                    edit(event, "**Sanitized runtime diagnostics**\n"
                            + "NeoMusicBot: `" + markdown(OtherUtil.getCurrentVersion()) + "`\n"
            ...
            {
                edit(event, fail("Diagnostics are unavailable while NeoMusicBot is shutting down."));
            }
        });
    }
    ...
    private DiagnosticSnapshot diagnosticSnapshot(MediaResolverReadiness readiness)
    {
        ...
        synchronized (diagnosticsLock)
        {
            ...
            String ytDlp = readiness.ytDlpAvailable()
                    ? probeVersion(executable("NEOMUSICBOT_YTDLP_PATH", "yt-dlp"), YT_DLP_VERSION)
                    : "missing";
            String deno = readiness.denoAvailable()
                    ? probeVersion(executable("NEOMUSICBOT_DENO_PATH", "deno"), DENO_VERSION)
                    : "missing";
    ...
    {
        String value = BotEnvironment.value(environmentName);
