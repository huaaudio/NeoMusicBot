...
AUD-005 已由最终两个平台 ZIP 的实际材料核验关闭；不能用 Maven 许可校验代替整份发行包的材料审查。
...
| 组成部分 | 当前交付与绑定证据 | 最终验收 |
| --- | --- | --- |
| Java 运行时依赖 | 44 个 SBOM 组件的许可证及 JAR 内原始 LICENSE/NOTICE 保留；原生组件另有固定材料 | 已通过：最终提交的实际两平台 ZIP 和 SBOM 复核 |
| yt-dlp 2026.08.19 | 两平台实际程序哈希、上游完整第三方声明与明确补充原文；86 份来源归档及构建/来源记录已通过独立附件公开回读；`runtime.ytdlp.notices`、`runtime.ytdlp.sources` 为强制门禁 | 已通过：最终完整 ZIP 的实际声明与公开源码访问验证 |
| Deno 2.9.7 | 固定程序、源码、原始声明及明确补充材料正式接入；共享源码附件 1077 项中包含 Deno 固定材料；不把 Cargo 完整锁图说成实际链接组件清单 | 已通过：最终完整 ZIP 中的程序绑定、原文检查和公开附件恢复验收 |
| bgutil provider 2.0.0 | 完整上游源码、原始及运行配置、GPL-3.0 文本、183 包运行安装/缓存清单及版本限定的补充原文；开发依赖不随新安装分发 | 已通过：最终两平台实际安装树与声明逐项比较 |
| QuickJS/WASM | 七个实际 WASM 的安装/缓存副本、编译器与固定源码/声明绑定；共享附件已不可变发布及公开回读 | 已通过：最终完整 ZIP 的 WASM 与恢复后源码验证 |
| Linux Canvas | 自建 37 文件原生集合及来源、源码、原始声明、构建输入固定为不可变 r2；实际库的源码分离回读通过 | 已通过：最终 Linux 完整应用 ZIP 及运行检查 |
| Windows Canvas | 保留官方配套库的评估已记录；402 项源码/构建文件的独立附件已不可变发布并公开回读，运行包保留原始声明 | 已通过：最终 Windows 完整应用 ZIP 及运行检查 |

当前不存在因附件尚未上传而缺失的源码下载地址。上表列出的最终 ZIP 验收已在发行提交 `3ff239c` 全部通过，AUD-005 已关闭。以下调查过程按发生时点保留，其中的“尚未接入”等历史状态以本表及末尾交付记录为准。
...
Deno 与 QuickJS 已接入共享源码附件分离：运行 ZIP 保留 `licenses/shared-source-access.json` 和原始声明；干净解压检查从固定公开地址下载附件，验证整体及逐文件哈希，再执行原有材料校验。共享附件 `deno-quickjs-sources-r1` 已不可变发布，草稿与公开下载的整体哈希、大小及标签提交关联均通过；实际 1077 个成员按当前正式源码清单逐文件核验。Linux Canvas 复用已发布的不可变原生材料 r2；Windows Canvas 使用单独的固定源码附件。两平台运行 ZIP 均保留原始声明和源码访问记录，验收在临时目录恢复经过哈希验证的源码后执行原有二进制、版本、来源和声明检查。Linux 实际原生库与 Windows 402 个源码文件的本地 ZIP 分离回读已通过；Windows 附件 `canvas-windows-sources-r1` 已不可变发布，草稿与公开读回哈希、大小和标签关联通过；最终应用完整 ZIP 验收仍待完成。历史 CI 外层传输 ZIP 已在确认其内层 ZIP 哈希与保留副本一致后清理；这些磁盘缓存不属于应用源码大小。
...
材料恢复逐项核对了公开附件、固定源码集合和实际程序/原生库/WASM；原始声明留在运行 ZIP，源码附件正常运行时无需下载。对应记录为 `tools/ci-success-3ff239c/verified-materials.json`。
