...
| 组成部分 | 当前交付与绑定证据 | 剩余验收 |
| --- | --- | --- |
| Java 运行时依赖 | 44 个 SBOM 组件的许可证及 JAR 内原始 LICENSE/NOTICE 保留；原生组件另有固定材料 | 最终提交的实际两平台 ZIP 和 SBOM 复核 |
| yt-dlp 2026.08.19 | 两平台实际程序哈希、上游完整第三方声明与明确补充原文；86 份来源归档及构建/来源记录已通过独立附件公开回读；`runtime.ytdlp.notices`、`runtime.ytdlp.sources` 为强制门禁 | 最终完整 ZIP 的实际声明与公开源码访问验证 |
| Deno 2.9.7 | 固定程序、源码、原始声明及明确补充材料正式接入；共享源码附件 1077 项中包含 Deno 固定材料；不把 Cargo 完整锁图说成实际链接组件清单 | 最终完整 ZIP 中的程序绑定、原文检查和公开附件恢复验收 |
| bgutil provider 2.0.0 | 完整上游源码、原始及运行配置、GPL-3.0 文本、183 包运行安装/缓存清单及版本限定的补充原文；开发依赖不随新安装分发 | 最终两平台实际安装树与声明逐项比较 |
| QuickJS/WASM | 七个实际 WASM 的安装/缓存副本、编译器与固定源码/声明绑定；共享附件已不可变发布及公开回读 | 最终完整 ZIP 的 WASM 与恢复后源码验证 |
| Linux Canvas | 自建 37 文件原生集合及来源、源码、原始声明、构建输入固定为不可变 r2；实际库的源码分离回读通过 | 最终 Linux 完整应用 ZIP 及运行检查 |
| Windows Canvas | 保留官方配套库的评估已记录；402 项源码/构建文件的独立附件已不可变发布并公开回读，运行包保留原始声明 | 最终 Windows 完整应用 ZIP 及运行检查 |

当前不存在因附件尚未上传而缺失的源码下载地址。AUD-005 保持开放，直到最终两平台完整 ZIP 的材料门禁和实物回读通过。以下调查过程按发生时点保留，其中的“尚未接入”等历史状态以本表及末尾交付记录为准。
