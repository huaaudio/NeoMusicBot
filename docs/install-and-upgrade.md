...
`/seek` 的时间可写为 `1:30`、`90` 或 `1m30s`；前置 `+` / `-` 表示相对当前位置前进或后退，
例如 `-10s`。秒数支持小数，按毫秒四舍五入。无效或超范围时间会被拒绝，不会截断或绕回到较小值；
最终位置仍必须位于当前可跳转曲目的时长范围内。

全局 Slash 命令在启动后注册。开发时可设置 `NEOMUSICBOT_COMMAND_GUILD_ID` 仅注册到一个服务器；
正式使用应移除它。旧前缀命令已不再注册。

本地歌单使用 UTF-8 文本，每行一项；空行以及 `#` / `//` 开头的注释不参与加载，
`#shuffle` 或 `//shuffle` 启用随机加载。owner 可用 `/owner playlist` 创建、追加和删除歌单。
追加会保留原有注释、指令与顺序，同一进程内的并发修改会依次保存。只识别普通 `.txt` 文件，
不跟随歌单文件的符号链接；已有中文或含空格的文件名仍可读取，命令创建的新名称使用安全字符。
保存使用同目录临时文件和原子替换；不支持原子替换的文件系统会报告失败并保留旧文件。
手动编辑歌单时请先停止机器人，避免外部编辑与程序保存互相覆盖。

全局 `skipratio` 接受 0 到 1 的有限数值（例如 0.55 表示 55%）；0 允许任一听众跳过，
1 要求所有合格听众投票。服务器设置中的 `skip_ratio` 另兼容 `-1`，表示继承全局值。
服务器 `volume` 必须是 0 到 150 的整数。越界、非有限或错误类型的值会拒绝加载；
服务器设置仍按既有规则尝试有效备份，无法恢复时保留原文件并停止启动。
...
启用 `updatealerts` 后，登录成功时及之后每 24 小时检查更新。测试版会检查后续已发布测试版
和正式版，正式版只检查正式版；草稿、无法识别的标签、网络错误均不会触发提醒。
更新检查不会自动下载或替换文件，关闭 `updatealerts` 可停用检查及提醒。

正常停止可使用 `/owner shutdown`、终端的 Ctrl+C 或关闭程序窗口。程序会关闭音源、保存待写设置，
并清理后台任务；服务管理器应留出至少 30 秒退出时间。强制结束进程或断电不能保证最后一次修改落盘。
